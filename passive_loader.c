#include <bpf/libbpf.h>
#include <sys/mman.h>
#include <fcntl.h>
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
#include <linux/if_link.h>
#include <net/if.h>

#include <bpf/bpf.h>

#include <sys/syscall.h>

#include <signal.h>
// libbpf 1.1.0+에선 <bpf/xdp.h>가 필요할 수도 있음
//#include <bpf/xdp.h>

static volatile bool exiting = false;

static void sig_handler(int sig)
{
    exiting = true;
}

static int run_ebpf_prog(int prog_fd)
{
    /* Look here: https://docs.kernel.org/bpf/bpf_prog_run.html */
    /* time_t before, after; */
    int ret;
    struct bpf_test_run_opts test_opts;
    memset(&test_opts, 0, sizeof(struct bpf_test_run_opts));
    /* TODO: optionally pass a context to it */
    test_opts.sz = sizeof(struct bpf_test_run_opts);
    test_opts.data_in = NULL;
    test_opts.data_size_in = 0;
    test_opts.ctx_in = NULL;
    test_opts.ctx_size_in = 0;
    test_opts.repeat = 0;
    test_opts.cpu = 0;
    /* before = read_tsc(); */
    ret = bpf_prog_test_run_opts(prog_fd, &test_opts); //BPF_PROG_TEST_RUN wrapper임
    /* after = read_tsc(); */
    if (ret < 0) {
        perror("something went wrong\n");
        return -1;
    }
    /*last_test_duration = test_opts.duration;*/
    return test_opts.retval;
}

int main(int argc, char **argv) {
    struct bpf_object *sys_obj = NULL;
    struct bpf_object *xdp_obj = NULL;
    struct bpf_program *prog = NULL;
    int prog_fd = -1, arena_fd = -1;
    const char *xdp_prog_name = "xdp_prog";      // arena_xdp_kern.c에서 SEC("xdp")의 함수명
    const char *syscall_prog_name = "arena_init";      // arena_xdp_kern.c에서 SEC("syscall")의 함수명
    const char *o_syscall_filename = "arena_syscall_kern.o";
    const char *o_xdp_filename = "arena_xdp_kern.o";
    const char *map_name = "arena_map";
    const char *iface = "ens1np0";
	
	signal(SIGINT, sig_handler);
    signal(SIGTERM, sig_handler);

	// 1. syscall 오브젝트 파일 열기
    sys_obj = bpf_object__open_file(o_syscall_filename, NULL);
    if (!sys_obj) {
        fprintf(stderr, "Failed to open syscall BPF object\n");
        return 1;
    }
    
	// 2. syscall 오브젝트 로드 (커널로 프로그램/맵 등록)
    if (bpf_object__load(sys_obj)) {
        fprintf(stderr, "Failed to load syscall BPF object\n");
        bpf_object__close(sys_obj);
        return 1;
    }
	
	//3. syscall 오브젝트 test run 호출
	struct bpf_program *init_prog = bpf_object__find_program_by_name(sys_obj, syscall_prog_name);
    if (!init_prog) {
        fprintf(stderr, "Couldn't find syscall program %s\n", syscall_prog_name);
        bpf_object__close(sys_obj);
        return 1;
    }
    int init_fd = bpf_program__fd(init_prog);
	printf("init_fd: %d\n", init_fd);	
	run_ebpf_prog(init_fd); //syscall test run 호출
    
	// 4. syscall arena_map FD 찾기
	struct bpf_map *map = bpf_object__find_map_by_name(sys_obj, map_name);
	int sys_fd = bpf_map__fd(map); //syscall의 arena_map fd 추출
    printf("syscall arena_fd %d\n", sys_fd);

    
	// 1. 오브젝트 파일 열기
    xdp_obj = bpf_object__open_file(o_xdp_filename, NULL);
    if (!xdp_obj) {
        fprintf(stderr, "Failed to open xdp BPF object\n");
        return 1;
    }
/**
    // 2. 오브젝트 로드 (커널로 프로그램/맵 등록)
    if (bpf_object__load(xdp_obj)) {
        fprintf(stderr, "Failed to load xdp BPF object\n");
        bpf_object__close(sys_obj);
        bpf_object__close(xdp_obj);
        return 1;
	}
	*/
	// 3. xdp arena_map FD 찾기
	struct bpf_map *xdp_map = bpf_object__find_map_by_name(xdp_obj, map_name);
	if(xdp_map == NULL) {
		printf("xdp_map == NULL\n");
	}
	int map_result = bpf_map__reuse_fd(xdp_map, sys_fd);
	if(map_result < 0) {
        fprintf(stderr, "Failed to reuse MAP\n");
        bpf_object__close(sys_obj);
        bpf_object__close(xdp_obj);
        return 1;
	}
    
	// 2. 오브젝트 로드 (커널로 프로그램/맵 등록)
    if (bpf_object__load(xdp_obj)) {
        fprintf(stderr, "Failed to load xdp BPF object\n");
        bpf_object__close(sys_obj);
        bpf_object__close(xdp_obj);
        return 1;
	}
	
	//adding for test
	//struct bpf_map *xdp_map = bpf_object__find_map_by_name(sys_obj, map_name);
	int xdp_fd = bpf_map__fd(xdp_map); //syscall의 arena_map fd 추출
    printf("xdp arena_fd %d\n", xdp_fd);
    
	// 3. XDP 프로그램 FD 추출 (함수명 주의: arena_xdp_kern.c에서 함수명 확인!)
    prog = bpf_object__find_program_by_name(xdp_obj, xdp_prog_name);
    if (!prog) {
        fprintf(stderr, "Couldn't find program %s\n", xdp_prog_name);
        //munmap(ptr, sz);
        //munmap(arena_data, sz);
        bpf_object__close(sys_obj);
        bpf_object__close(xdp_obj);
        return 1;
    }
    prog_fd = bpf_program__fd(prog);

    // 4. NIC에 XDP 프로그램 attach
    int ifindex = if_nametoindex(iface);
    if (!ifindex) {
        fprintf(stderr, "Invalid interface: %s\n", iface);
        //munmap(ptr, sz);
        //munmap(arena_data, sz);
        bpf_object__close(sys_obj);
        bpf_object__close(xdp_obj);
        return 1;
    }

	__u32 xdp_flags = 0;
	xdp_flags |= XDP_FLAGS_DRV_MODE;
    int ret = bpf_xdp_attach(ifindex, prog_fd, xdp_flags, NULL);
    if (ret < 0) {
        perror("bpf_xdp_attach");
        //munmap(ptr, sz);
        //munmap(arena_data, sz);
        bpf_object__close(sys_obj);
        bpf_object__close(xdp_obj);
        return 1;
    }
	else {
		printf("program attach %d\n", ifindex);
	}

    //printf("XDP loaded. Initial arena byte = %u\n", ptr[0]);
    //printf("XDP loaded. Initial arena byte = %p\n", arena_data);
    while (!exiting) sleep(10);

	bpf_xdp_detach(ifindex, xdp_flags, NULL);
	bpf_object__close(xdp_obj);
    bpf_object__close(sys_obj);

	printf("program detach %d\n", ifindex);
    // detach/clean-up (생략)
    return 0;
}

